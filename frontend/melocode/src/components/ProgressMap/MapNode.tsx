...
export function MapNode({ node, index }: MapNodeProps) {
  const statusClasses: Record<Node["status"], string> = {
    ...
    current:
      "bg-[var(--accent-5)] border-[var(--accent-8)] text-[var(--accent-11)] animate-pulse hover:border-b-4 hover:mb-1 active:border-b-1 active:mb-[1px]",
    completed:
  ...
  return (
    <Tooltip content={node.title}>
      <Link
        ...
        to={node.status === "locked" ? "#" : node.slug}
      >
